        ...
        options: {
            ...
            scales: {
                x: {
                    ...
                    min: 0,
                    max: 50, // Fixed max to match latency chart
                    ticks: {
...
function updateThroughputChart(chart) {
    ...
    if (allDownloadData.length > 0) {
        ...
        console.log(`Dynamic download y-axis: max data=${maxDownload.toFixed(2)}, suggestedMax=${suggestedMax}`);
    } else {
    ...
    if (allUploadData.length > 0) {
        ...
        console.log(`Dynamic upload y-axis: max data=${maxUpload.toFixed(2)}, suggestedMax=${suggestedMax}`);
    } else {
...
function addNullDownloadDataPoint(chart, seconds) {
    ...
    console.log(`Added download break points at ${seconds}s`);
    chart.update('none');
}
...
function addNullUploadDataPoint(chart, seconds) {
    ...
    console.log(`Added upload break points at ${seconds}s`);
    chart.update('none');
}
...
function updateThroughputChartWithAllData(chart, downloadData, uploadData, phaseTransitions = []) {
    ...
    console.log('Using phase transitions from phase controller:', phaseTransitions);
...
function addDataPointsWithBreaks(chart, datasetIndex, data, transitions) {
    ...
    for (let i = 0; i < sortedData.length; i++) {
        ...
        if (nearestTransition) {
            ...
            console.log(`Added break at phase transition: ${nearestTransition.time}s (${nearestTransition.fromPhase} → ${nearestTransition.toPhase})`);
        }
    ...
    for (const [transitionTime, transition] of transitionTimes.entries()) {
        if (transitionTime > lastAddedTransitionTime && sortedData.length > 0) {
            ...
            if (transitionTime > lastPointTime && transitionTime - lastPointTime < 5) {
                ...
                console.log(`Added break at phase transition after data: ${transition.time}s (${transition.fromPhase} → ${transition.toPhase})`);
            }
        }
    }
}
