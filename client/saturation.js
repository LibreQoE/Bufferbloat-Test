...
let cumulativeBytesReceived = 0; // Cumulative bytes for download
let cumulativeBytesSent = 0; // Cumulative bytes for upload
let lastThroughputResetTime = 0; // Last time we reset cumulative counters
const CUMULATIVE_RESET_INTERVAL = 10000; // Reset cumulative counters every 10 seconds to prevent overflow
...
async function startDownloadSaturation(isDiscovery = false, fixedThroughput = 0, optimalParams = null) {
    ...
    if (!isDiscovery) {
        ...
        // Reset cumulative counters and smoothed values
...
async function startUploadSaturation(isDiscovery = false, fixedThroughput = 0, optimalParams = null) {
    ...
    if (optimalParams && optimalParams.streamCount > 0) {
        ...
        if (isAsymmetricConnection) {
            ...
            // Use more streams with less aggressive parameters
            MAX_UPLOAD_STREAMS = Math.max(4, Math.min(8, Math.ceil(fixedThroughput / 5)));
            ...
            // Scale streams from 2 to 12 based on logarithmic throughput scale
            // This ensures we don't use too many streams for low bandwidth
            // but still scale up appropriately for very high bandwidth
            const streamCount = Math.max(2, Math.min(12, Math.round(
                2 + 10 * logScale
            )));
        ...
        if (fixedThroughput > 1000 && !isAsymmetricConnection) {
            ...
            // Use more streams with smaller chunks and more aggressive pacing
            MAX_UPLOAD_STREAMS = Math.min(16, Math.max(8, Math.ceil(MAX_UPLOAD_STREAMS * 1.5)));
        ...
        // Default full test settings with more gradual ramp-up
        MAX_UPLOAD_STREAMS = 8; // Fixed to 8 streams as requested
...
function stopAllStreams() {
    ...
    // Initialize the throughput reset time
    lastThroughputResetTime = performance.now();
...
function measureDownloadThroughput() {
    ...
    cumulativeBytesReceived += intervalBytes;
    
    // Periodically reset cumulative counters to prevent overflow
    if (now - lastThroughputResetTime > CUMULATIVE_RESET_INTERVAL) {
        cumulativeBytesReceived = intervalBytes;
        lastThroughputResetTime = now;
    }
...
function measureUploadThroughput() {
    ...
    // Add to cumulative counter
    cumulativeBytesSent += intervalBytes;
    
    // Periodically reset cumulative counters to prevent overflow
    if (now - lastThroughputResetTime > CUMULATIVE_RESET_INTERVAL) {
        cumulativeBytesSent = intervalBytes;
        // Note: lastThroughputResetTime is already updated in the download function
    }
